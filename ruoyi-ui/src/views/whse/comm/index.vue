<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="商品状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="商品状态" clearable>
          <el-option
            v-for="dict in dict.type.whse_comm_status"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-table v-loading="loading" :data="commList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="商品编号" align="center" prop="commId" />
      ...
      <el-table-column label="商品状态" align="center" prop="status">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.whse_comm_status" :value="scope.row.status"/>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="商品单价" prop="commPrice">
          <el-input v-model="form.commPrice" placeholder="请输入商品单价" />
        </el-form-item>

        <el-form-item label="商品类型" prop="commType">
...
<script>
import { listComm, getComm, delComm, addComm, updateComm } from "@/api/whse/comm";

export default {
  name: "Comm",
  dicts: ['whse_comm_status','whse_comm_types'],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除商品编号为"' + commIds + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('whse/comm/export', {
        ...this.queryParams
